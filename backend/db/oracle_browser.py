...
def switch_identity_to_default(conn, schema: str, table: str) -> list[str]:
    """Switch IDENTITY columns from GENERATED ALWAYS to GENERATED BY DEFAULT.

    This allows explicit INSERT values (needed for migration).
    Returns list of column names that were switched.
    """
    s = schema.upper()
    t = table.upper()
    switched: list[str] = []
        ...
        for col_name, gen_type in rows:
            if gen_type == "ALWAYS":
                try:
                    ...
                    switched.append(col_name)
                except Exception as exc:
                    print(f"[oracle_browser] could not switch identity "
                          f"{s}.{t}.{col_name}: {exc}")
    conn.commit()
    return switched
