...
/** Per-table key configuration block (chips + key-type buttons + column picker). */
export function TableKeyConfig({ entry, onChange }: Props) {
  ...
  if (entry.loadingInfo) {
    return (
      <div style={{ fontSize: t.size.sm, color: t.text.disabled, padding: "4px 0" }}>
        Загрузка информации...
      </div>
    );
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      {entry.effective_key_type === "PRIMARY_KEY" && (
        <div style={{ fontSize: t.size.sm, color: t.green.fg }}>
      ...
      {entry.effective_key_type === "UNIQUE_KEY" && (
        <>
          {info.uk_constraints.length > 1 && (
            <select
              style={{ ...S.select, fontSize: t.size.sm }}
          ...
          <div style={{ fontSize: t.size.sm, color: t.purple.fg }}>
          ...
          <div style={{
            ...
            {info.columns.map(col => (
              <label key={col.name} style={{
                ...
                />
                <span style={{ fontSize: t.size.sm, color: t.text.primary, fontFamily: t.font.mono }}>
                  {col.name}
                </span>
